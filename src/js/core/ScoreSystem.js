...
export class ScoreSystem {
  constructor() {
    ...
    console.log(`[ScoreSystem] 加载历史最高分: ${this.highScores.join(', ')}`);
  }
  ...
  calculateScore(isHit, stats, timeLeft, totalTime) {
    ...
    console.log(
      `[ScoreSystem] 分数更新: ${oldScore} -> ${this.gameData.score} ` +
  ...
  saveHighScore() {
    console.log(`[ScoreSystem] 保存高分: ${this.gameData.score}`);
    this.highScores.push(this.gameData.score);
    this.highScores.sort((a, b) => b - a);
    this.highScores = this.highScores.slice(0, 5);

    console.log(
      `[ScoreSystem] 更新后的高分排行: ${this.highScores.join(', ')}`
    );
